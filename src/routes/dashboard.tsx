...
function renderBody(body: string) {
  if (!body) return <span className="text-[#71717a]">—</span>;
    ...
    <ul className="space-y-2">
      {lines.map((line, i) => {
        ...
        const clean = line
          ...
          .replace(/\*(.+?)\*/g, "$1");
        return (
          <li key={i} className="flex gap-2.5 text-sm text-[#a1a1aa] leading-relaxed">
            {isBullet && <span className="shrink-0 w-1.5 h-1.5 rounded-full bg-[#6366f1]/50 mt-[7px]" />}
...
Sample rows (first 10 + last 5): ${JSON.stringify(sample)}`;
  ...
  const cardMeta = sections ? [
    { title: "Summary",         icon: FileText,    body: sections.summary,         color: "#6366f1" },
    ...
    <div className="py-8">
      <div className="flex flex-wrap items-start justify-between gap-4 page-enter page-enter-stagger-1">
        <div className="hero-spotlight">
          <h1 className="text-3xl font-bold text-[#e8e8f0]">Dashboard</h1>
          <p className="text-sm text-[#71717a]">AI-powered analysis of your dataset.</p>
        </div>
        <div className="flex gap-2">
          ...
          {sections && (
            <Button onClick={downloadPDF} variant="outline"
              className="border-[#2a2a3e] text-[#a1a1aa] hover:bg-[#1a1a2e] hover:text-white"
            >
      ...
      {!hasData && !loading && !sections && (
        ...
        >
          ...
          <div className="gradient-bg flex h-16 w-16 items-center justify-center rounded-2xl shadow-[0_0_40px_-8px_rgba(99,102,241,0.5)]">
            <BarChart3 className="h-8 w-8 text-white" />
          </div>
          <h2 className="mt-6 text-xl font-semibold text-[#e8e8f0]">Upload data to begin</h2>
      ...
      {hasData && !sections && !loading && (
        <div className="glass-card mt-4 p-6 text-sm text-[#a1a1aa] page-enter page-enter-stagger-3">
          {report?.cleanedRows.toLocaleString()} rows · {columns.length} columns ready.
          <span className="ml-1 inline-flex items-center gap-1 text-[#6366f1]">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="glass-card p-6 page-enter" style={{ animationDelay: `${i * 80}ms` }}>
              <Skeleton className="h-5 w-32 bg-[#1a1a2e]" />
              <div className="mt-4 space-y-2">
                <Skeleton className="h-3 w-full bg-[#1a1a2e]" />
                <Skeleton className="h-3 w-5/6 bg-[#1a1a2e]" />
                <Skeleton className="h-3 w-4/6 bg-[#1a1a2e]" />
                <Skeleton className="h-3 w-3/6 bg-[#1a1a2e]" />
              </div>
            </div>
          ))}
        </div>
          ...
          {cardMeta.map(({ title, icon: Icon, body, color }, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3 relative z-10">
                ...
                <h3 className="text-sm font-semibold text-[#e8e8f0] uppercase tracking-wide">{title}</h3>
              </div>
