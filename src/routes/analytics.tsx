    ...
    <div className="py-8">
      <div className="flex items-center justify-between mb-6 page-enter page-enter-stagger-1">
        <div className="hero-spotlight">
          <h1 className="text-3xl font-bold text-[#f0f0f0]">Analytics</h1>
          ...
          {showNameInput ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded-lg border border-white/10 bg-[#111111] px-3 py-1.5 text-xs text-white outline-none focus:border-purple-500 w-40"
              />
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 md:grid-cols-5 page-enter page-enter-stagger-2">
          {[
            ...
            >
              ...
              <div className="text-xs text-[#555] uppercase tracking-wider mb-1 font-mono relative z-10">{kpi.label}</div>
            ...
            {categoricalCols.slice(0, 6).map(col => (
              <div key={col}>
                ...
                <select value={filters[col] ?? ""} onChange={e => setFilters(f => ({ ...f, [col]: e.target.value }))}
                  className="w-full rounded-lg border border-white/10 bg-[#111111] px-3 py-2 text-xs text-white outline-none focus:border-purple-500">
            ...
            <ResponsiveContainer width="100%" height={showDataTable ? 420 : 620}>
              {chartType === "bar" ? (
                <BarChart data={chartData} onClick={canDrillDown ? handleDrillDown : undefined} style={canDrillDown ? { cursor: "pointer" } : {}}>
                  ...
                  <XAxis dataKey="name" tick={({ x, y, payload }) => (<text x={x} y={y + 8} textAnchor="end" transform={`rotate(-35, ${x}, ${y + 8})`} fill="#555" fontSize={10} fontFamily="'JetBrains Mono', monospace">{String(payload.value).length > 12 ? String(payload.value).slice(0, 12) + "…" : payload.value}</text>)} height={60} interval={0} />
                  <YAxis tick={{ fill: "#555", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }} />
                ...
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" stroke="#111111" />
                  <XAxis dataKey="x" name={activeX} tick={{ fill: "#555", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }} />
                  <YAxis dataKey="y" name={activeY} tick={{ fill: "#555", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }} />
            ...
            <div key={col} className="glass-card p-4" id={`secondary-chart-${idx}`}>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={data} barSize={20}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#111111" />
                  <XAxis dataKey="name" tick={{ fill: "#555", fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} />
                  <YAxis tick={{ fill: "#555", fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} />
                  <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} />
