      ...
      {text.split("\n").map((line, i) => {
        ...
        while ((match = pattern.exec(remaining)) !== null) {
          ...
          if (match[0].startsWith("**")) {
            ...
            parts.push(
              <code key={key++} className="bg-white/10 px-1 rounded text-xs font-mono">
                {match[4]}
              </code>
            );
          } else {
...
Use plain text formatting only — no markdown asterisks or symbols.`;
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col py-8">
      ...
      <div className="glass-card mt-2 flex flex-1 flex-col overflow-hidden">
        <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-6">
          {messages.length === 0 && !loading && (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <div className="gradient-bg flex h-14 w-14 items-center justify-center rounded-2xl shadow-[var(--shadow-glow)]">
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              >
                {m.role === "assistant" ? <SafeMarkdown text={m.content} /> : m.content}
              </div>
            </div>
          ))}
        ...
        <div className="flex gap-2 border-t border-white/10 p-4">
          <Input
            ...
            disabled={loading}
          />
          <Button onClick={send} disabled={loading || !input.trim()} className="gradient-bg">
