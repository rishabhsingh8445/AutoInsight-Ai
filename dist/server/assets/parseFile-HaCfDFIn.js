import { $ as getDefaultExportFromCjs } from "./worker-entry-klvM4nWa.js";
...
function normalizeColumnName(col) {
  return col.trim().replace(/\s+/g, " ").replace(
    /\w\S*/g,
...
async function downloadCSV(columns, rows, fileName = "cleaned_data.xlsx") {
  const ExcelJS = await import("./excel-5H0Vdcz7.js").then((n) => n.e);
      ...
      if (numericCols.has(col)) {
        ...
        if (stats && stats.max !== stats.min) {
          ...
          if (!isNaN(num)) {
            ...
            let argb = "FFEF4444";
            if (ratio >= 0.75) argb = "FF22C55E";
            else if (ratio >= 0.45) argb = "FFEAB308";
