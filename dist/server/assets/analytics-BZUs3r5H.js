import { a1 as commonjsGlobal, $ as getDefaultExportFromCjs, r as reactExports, S as React, T as jsxRuntimeExports } from "./worker-entry-klvM4nWa.js";
...
const BookmarkCheck = createLucideIcon("bookmark-check", __iconNode$8);
const __iconNode$7 = [
  [
    "path",
    {
...
const ImageDown = createLucideIcon("image-down", __iconNode$4);
const __iconNode$3 = [
  [
    "path",
    {
...
const Table2 = createLucideIcon("table-2", __iconNode$3);
const __iconNode$2 = [
...
function getTooltipCSSClassName(_ref) {
  ...
  return clsx(CSS_CLASS_PREFIX, _defineProperty$F(_defineProperty$F(_defineProperty$F(_defineProperty$F({}, "".concat(CSS_CLASS_PREFIX, "-right"), isNumber(translateX) && coordinate && isNumber(coordinate.x) && translateX >= coordinate.x), "".concat(CSS_CLASS_PREFIX, "-left"), isNumber(translateX) && coordinate && isNumber(coordinate.x) && translateX < coordinate.x), "".concat(CSS_CLASS_PREFIX, "-bottom"), isNumber(translateY) && coordinate && isNumber(coordinate.y) && translateY >= coordinate.y), "".concat(CSS_CLASS_PREFIX, "-top"), isNumber(translateY) && coordinate && isNumber(coordinate.y) && translateY < coordinate.y));
}
...
var parseLabel = function parseLabel2(label, viewBox) {
  ...
  if (isObject(label)) {
    return /* @__PURE__ */ React.createElement(Label, _extends$p({
      viewBox
    }, label, {
      key: "label-implicit"
    }));
...
function LabelList(_ref) {
  ...
  return /* @__PURE__ */ React.createElement(Layer, {
    ...
    return /* @__PURE__ */ React.createElement(Label, _extends$o({}, filterProps(entry, true), others, idProps, {
...
var PolarRadiusAxis = /* @__PURE__ */ (function(_PureComponent) {
  ...
  return _createClass$g(PolarRadiusAxis2, [{
    ...
    value: function renderAxisLine() {
      var _this$props3 = this.props, cx = _this$props3.cx, cy = _this$props3.cy, angle = _this$props3.angle, ticks2 = _this$props3.ticks, axisLine = _this$props3.axisLine, others = _objectWithoutProperties$8(_this$props3, _excluded$8);
...
function getCategoricalColumns(columns, rows) {
  return columns.filter((col) => {
    const vals = rows.slice(0, 20).map((r) => r[col]);
...
function buildChartTitle(chartType, activeX, activeY, filters, drillStack) {
  ...
  if (drillStack.length > 0) {
    return {
      title,
      subtitle: `Drill: ${drillStack.map((d) => `${d.col} = ${d.value}`).join(" → ")}`
    };
  }
  let subtitle = "All data";
  if (activeFilters.length === 1) subtitle = `Filtered by ${activeFilters[0][0]} = ${activeFilters[0][1]}`;
  else if (activeFilters.length > 1) subtitle = `${activeFilters.length} filters active: ${activeFilters.map(([k2, v]) => `${k2}=${v}`).join(", ")}`;
  return {
    title,
    subtitle
  };
...
const downloadChart = (id, name) => {
  ...
  let maxArea = 0;
  svgs.forEach((s2) => {
    const area = s2.clientWidth * s2.clientHeight;
    if (area > maxArea) {
      maxArea = area;
      svg = s2;
    }
...
const downloadSnapshotImage = (snapshotName, chartTitle, chartSubtitle, chartType, filters, drillStack, kpis, chartData) => {
  const SCALE = 2;
  const W = 1200;
  const H = 800;
  const cW = W * SCALE;
  ...
  ctx.textAlign = "right";
  ctx.fillText((/* @__PURE__ */ new Date()).toLocaleString(), W - 32, 32);
  ctx.textAlign = "left";
  ...
  if (kpis) {
    const kpiItems = [{
      ...
      label: `Sum (${kpis.col})`,
      value: Number(kpis.sum).toLocaleString(),
      color: "#818cf8"
    }, {
      label: "Average",
      value: kpis.avg,
      ...
      label: "Min",
      value: String(kpis.min),
      color: "#4ade80"
    }, {
      label: "Max",
      value: String(kpis.max),
      color: "#c084fc"
    }];
    const cardW = (W - 64 - 16 * 4) / 5;
    kpiItems.forEach((k2, i) => {
      ...
      ctx.fillText(k2.label.toUpperCase(), cx + 10, cy + 18);
      ctx.fillStyle = k2.color;
      ctx.font = "bold 18px system-ui, sans-serif";
      ctx.fillText(k2.value, cx + 10, cy + 44);
    });
  }
  const activeFilters = Object.entries(filters).filter(([, v]) => Boolean(v));
  const drillPath = drillStack.map((d) => `${d.col}=${d.value}`).join(" → ");
  const filterStr = [...activeFilters.map(([k2, v]) => `${k2}: ${v}`), ...drillPath ? [`Drill: ${drillPath}`] : []].join("  |  ");
  if (filterStr) {
...
function roundRect(ctx, x2, y2, w, h, r) {
  ctx.beginPath();
  ctx.moveTo(x2 + r, y2);
...
function drawChartFallback(ctx, chartData, chartType, x2, y2, w, h) {
  ...
  chartData.slice(0, 15).forEach((d, i) => {
    ...
    const bx = x2 + 20 + i * (barW + 8);
    const by = y2 + barAreaH - barH;
    const color2 = COLORS[i % COLORS.length];
    ...
    ctx.fillText(label, bx, y2 + barAreaH + 14);
  });
}
function drawFooterAndSave(ctx, canvas, W, H, snapshotName, chartType, chartData) {
  ctx.strokeStyle = "rgba(255,255,255,0.06)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(32, H - 44);
  ctx.lineTo(W - 32, H - 44);
  ctx.stroke();
  ctx.fillStyle = "rgba(148,163,184,0.5)";
  ctx.font = "10px system-ui, sans-serif";
  const summary = chartType !== "scatter" ? `${chartData.length} categories  ·  Chart: ${chartType}` : `${chartData.length} data points  ·  Chart: scatter`;
...
function AnalyticsPage() {
  ...
  const [xCol, setXCol] = reactExports.useState("");
  const [yCol, setYCol] = reactExports.useState("");
  const [showDataTable, setShowDataTable] = reactExports.useState(false);
  const [drillStack, setDrillStack] = reactExports.useState([]);
  ...
  const [savedFeedback, setSavedFeedback] = reactExports.useState(false);
  const [downloading, setDownloading] = reactExports.useState(false);
  const numericCols = reactExports.useMemo(() => getNumericColumns(columns, rows), [columns, rows]);
  const categoricalCols = reactExports.useMemo(() => getCategoricalColumns(columns, rows), [columns, rows]);
  const filteredRows = reactExports.useMemo(() => {
    return rows.filter((row) => {
      const passesFilters = Object.entries(filters).every(([col, val]) => !val || String(row[col] ?? "").toLowerCase().includes(val.toLowerCase()));
      const passesDrill = drillStack.every(({
        col,
        value
  ...
  const kpis = reactExports.useMemo(() => {
    ...
    return {
      ...
      sum: vals.reduce((a2, b) => a2 + b, 0).toFixed(2),
      avg: (vals.reduce((a2, b) => a2 + b, 0) / vals.length).toFixed(2),
      min: Math.min(...vals),
      max: Math.max(...vals),
      col: numCol
    };
  ...
  const nextDrillCol = reactExports.useMemo(() => {
    const usedCols = /* @__PURE__ */ new Set([activeX, ...drillStack.map((d) => d.col)]);
    return categoricalCols.find((c2) => !usedCols.has(c2)) ?? null;
  }, [categoricalCols, activeX, drillStack]);
  const chartData = reactExports.useMemo(() => {
    if (!activeX) return [];
    if (chartType === "scatter") {
      return filteredRows.slice(0, 200).map((r) => ({
        x: Number(r[activeX] ?? 0),
        y: Number(r[activeY] ?? 0)
      }));
    }
    if (chartType === "pie") {
      const counts2 = {};
      filteredRows.forEach((r) => {
        const v = String(r[activeX] ?? "Unknown");
        counts2[v] = (counts2[v] ?? 0) + 1;
      });
      return Object.entries(counts2).slice(0, 8).map(([name, value]) => ({
        name,
        value
      }));
    }
    ...
    return Object.entries(counts).slice(0, 15).map(([name, value]) => ({
      name,
      value
    }));
  ...
  } = reactExports.useMemo(() => buildChartTitle(chartType, activeX, activeY, filters, drillStack), [chartType, activeX, activeY, filters, drillStack]);
  ...
  const saveBookmark = () => {
    ...
    setTimeout(() => setSavedFeedback(false), 2e3);
  };
  const loadBookmark = (bm) => {
    setFilters(bm.filters);
    setChartType(bm.chartType);
    setXCol(bm.xCol);
    setYCol(bm.yCol);
    setDrillStack(bm.drillStack);
  ...
  const handleSnapshotDownload = (bm) => {
    setDownloading(true);
    setTimeout(() => {
      downloadSnapshotImage(bm.name, chartTitle, chartSubtitle, bm.chartType, bm.filters, bm.drillStack, kpis, chartData);
  ...
  const handleCurrentSnapshotDownload = () => {
    ...
    setDownloading(true);
    setTimeout(() => {
      downloadSnapshotImage(name, chartTitle, chartSubtitle, chartType, filters, drillStack, kpis, chartData);
      setDownloading(false);
    }, 100);
  };
  ...
  if (!columns.length) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mx-auto max-w-2xl py-20 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-12", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-2 text-muted-foreground", children: "Upload a file to see analytics." }),
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "py-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-3xl font-bold", children: "Analytics" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-muted-foreground", children: "Interactive data exploration" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: handleCurrentSnapshotDownload, disabled: downloading, className: "flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-white/5 transition-all disabled:opacity-50", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ImageDown, { className: "h-3 w-3" }),
          downloading ? "Exporting..." : "Export HD"
        ] }),
        showNameInput ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { autoFocus: true, value: bookmarkName, onChange: (e) => setBookmarkName(e.target.value), onKeyDown: (e) => {
            if (e.key === "Enter") saveBookmark();
            if (e.key === "Escape") setShowNameInput(false);
          }, placeholder: "Snapshot name...", className: "rounded-lg border border-white/10 bg-[#1a1a2e] px-3 py-1.5 text-xs text-white outline-none focus:border-purple-500 w-40" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: saveBookmark, className: "gradient-bg rounded-lg px-3 py-1.5 text-xs font-medium text-white", children: "Save" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setShowNameInput(false), className: "text-xs text-muted-foreground hover:text-white", children: "Cancel" })
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => setShowNameInput(true), className: `flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${savedFeedback ? "border-green-500/50 bg-green-500/10 text-green-400" : "border-white/10 text-muted-foreground hover:bg-white/5"}`, children: [
          savedFeedback ? /* @__PURE__ */ jsxRuntimeExports.jsx(BookmarkCheck, { className: "h-3 w-3" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Bookmark, { className: "h-3 w-3" }),
          savedFeedback ? "Saved!" : "Save Snapshot"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => setShowBookmarks((v) => !v), className: `flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${showBookmarks ? "border-purple-500/50 bg-purple-500/10 text-purple-300" : "border-white/10 text-muted-foreground hover:bg-white/5"}`, children: [
    ...
    showBookmarks && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4 mb-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-sm font-semibold mb-3", children: "Saved Snapshots" }),
      bookmarks.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "No snapshots saved yet." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-2 sm:grid-cols-2 lg:grid-cols-3", children: bookmarks.map((bm) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between rounded-lg border border-white/10 bg-white/5 p-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs font-semibold truncate", children: bm.name }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] text-muted-foreground mt-0.5", children: bm.createdAt }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-[10px] text-purple-400 mt-1", children: [
            bm.chartType,
            " · ",
            bm.xCol,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 ml-2 shrink-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => loadBookmark(bm), className: "rounded bg-purple-500/20 px-2 py-1 text-[10px] font-medium text-purple-300 hover:bg-purple-500/30", children: "Load" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => handleSnapshotDownload(bm), title: "Download HD snapshot", className: "rounded p-1 text-muted-foreground hover:text-purple-400", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ImageDown, { className: "h-3 w-3" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => deleteBookmark(bm.id), className: "rounded p-1 text-muted-foreground hover:text-red-400", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "h-3 w-3" }) })
        ] })
      ] }, bm.id)) })
    ] }),
    kpis && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-4 mb-6 md:grid-cols-5", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground uppercase tracking-wider mb-1", children: "Total Rows" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-bold gradient-text", children: kpis.total.toLocaleString() })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-muted-foreground uppercase tracking-wider mb-1", children: [
          "Sum (",
          kpis.col,
          ")"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-bold", children: Number(kpis.sum).toLocaleString() })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground uppercase tracking-wider mb-1", children: "Average" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-bold", children: kpis.avg })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground uppercase tracking-wider mb-1", children: "Min" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-bold text-green-400", children: kpis.min })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground uppercase tracking-wider mb-1", children: "Max" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-bold text-purple-400", children: kpis.max })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 gap-6 lg:grid-cols-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4 lg:col-span-1", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
          categoricalCols.slice(0, 6).map((col) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-xs text-muted-foreground mb-1 block", children: col }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: filters[col] ?? "", onChange: (e) => setFilters((f) => ({
              ...f,
              [col]: e.target.value
            })), className: "w-full rounded-lg border border-white/10 bg-[#1a1a2e] px-3 py-2 text-xs text-white outline-none focus:border-purple-500", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "All" }),
              [...new Set(rows.map((r) => String(r[col] ?? "")))].filter(Boolean).slice(0, 20).map((v) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: v, children: v }, v))
            ] })
          ] }, col)),
          Object.values(filters).some(Boolean) && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setFilters({}), className: "w-full text-xs text-red-400 hover:text-red-300 mt-2", children: "Clear all filters" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "text-sm font-semibold mt-6 mb-4 flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 gap-1", children: ["bar", "line", "pie", "scatter"].map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => {
              setChartType(t);
              setDrillStack([]);
            }, className: `rounded-lg px-2 py-1.5 text-xs font-medium capitalize transition-all ${chartType === t ? "gradient-bg text-white" : "border border-white/10 text-muted-foreground hover:bg-white/5"}`, children: t }, t)) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-xs text-muted-foreground mb-1 block", children: "X Axis" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: activeX, onChange: (e) => {
              setXCol(e.target.value);
              setDrillStack([]);
            }, className: "w-full rounded-lg border border-white/10 bg-[#1a1a2e] px-3 py-2 text-xs text-white outline-none focus:border-purple-500", children: columns.map((c2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: c2, children: c2 }, c2)) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-xs text-muted-foreground mb-1 block", children: "Y Axis" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: activeY, onChange: (e) => setYCol(e.target.value), className: "w-full rounded-lg border border-white/10 bg-[#1a1a2e] px-3 py-2 text-xs text-white outline-none focus:border-purple-500", children: columns.map((c2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: c2, children: c2 }, c2)) })
          ] }),
          canDrillDown && drillStack.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[10px] text-purple-400 mt-1", children: "💡 Click a bar/slice to drill down" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4 lg:col-span-3", children: [
        drillStack.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 mb-3 flex-wrap", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => handleDrillUp(0), className: "text-xs text-purple-400 hover:text-purple-300 font-medium", children: "All data" }),
          drillStack.map((d, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "flex items-center gap-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { className: "h-3 w-3 text-muted-foreground" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => handleDrillUp(i + 1), className: `text-xs font-medium ${i === drillStack.length - 1 ? "text-white" : "text-purple-400 hover:text-purple-300"}`, children: [
              d.col,
              " = ",
              d.value
            ] })
          ] }, i)),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => handleDrillUp(0), className: "ml-auto flex items-center gap-1 rounded border border-white/10 px-2 py-0.5 text-xs text-muted-foreground hover:bg-white/5", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between mb-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-0.5", children: chartSubtitle })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => setShowDataTable((v) => !v), className: `flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs font-medium transition-all ${showDataTable ? "border-purple-500/50 bg-purple-500/10 text-purple-300" : "border-white/10 text-muted-foreground hover:bg-white/5"}`, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Table2, { className: "h-3 w-3" }),
              showDataTable ? "Hide Data" : "Show Data"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => downloadChart("main-chart", "analytics-chart"), className: "flex items-center gap-1 text-xs text-muted-foreground hover:text-white", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Download, { className: "h-3 w-3" }),
              " PNG"
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "main-chart", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: showDataTable ? 420 : 620, children: chartType === "bar" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data: chartData, onClick: canDrillDown ? handleDrillDown : void 0, style: canDrillDown ? {
          cursor: "pointer"
        } : {}, children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: tooltipStyle, itemStyle: tooltipItemStyle, formatter: (value) => [value, "Count"], labelFormatter: (label) => canDrillDown ? `${label} (click to drill down)` : label }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "value", radius: [4, 4, 0, 0], children: chartData.map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: COLORS[i % COLORS.length] }, i)) })
        ] }) : chartType === "line" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(LineChart, { data: chartData, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "#ffffff10" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(XAxis, { dataKey: "name", tick: {
            fill: "#94a3b8",
            fontSize: 11
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: tooltipStyle, itemStyle: tooltipItemStyle }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Line, { type: "monotone", dataKey: "value", stroke: "#8b5cf6", strokeWidth: 2, dot: false })
        ] }) : chartType === "pie" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Pie, { data: chartData, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", outerRadius: 110, label: true, onClick: canDrillDown ? (data) => handleDrillDown({
            name: data.name,
            activePayload: [{
              payload: {
                name: data.name
          ...
          }) : void 0, style: canDrillDown ? {
            cursor: "pointer"
          } : {}, children: chartData.map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: COLORS[i % COLORS.length] }, i)) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: tooltipStyle, itemStyle: tooltipItemStyle, formatter: (value, name) => [value, canDrillDown ? `${name} (click to drill)` : name] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Legend, {})
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(ScatterChart, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "#ffffff10" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(XAxis, { dataKey: "x", name: activeX, tick: {
            fill: "#94a3b8",
            fontSize: 11
          } }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(YAxis, { dataKey: "y", name: activeY, tick: {
            fill: "#94a3b8",
            fontSize: 11
          } }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: tooltipStyle, itemStyle: tooltipItemStyle, cursor: {
            strokeDasharray: "3 3"
          } }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Scatter, { data: chartData, fill: "#a78bfa" })
        ] }) }) }),
        showDataTable && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3 overflow-hidden rounded-lg border border-white/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto max-h-52 overflow-y-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full text-xs", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { className: "sticky top-0 bg-[#0f0f1a]", children: /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: chartType === "scatter" ? [activeX, activeY].map((h) => /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "border-b border-white/10 px-3 py-2 text-left font-semibold text-muted-foreground", children: h }, h)) : ["Name", "Value"].map((h) => /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "border-b border-white/10 px-3 py-2 text-left font-semibold text-muted-foreground", children: h }, h)) }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: chartData.map((row, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { className: "border-b border-white/5 hover:bg-white/5", children: chartType === "scatter" ? [row.x, row.y].map((v, j) => /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-3 py-1.5 font-mono text-muted-foreground", children: String(v) }, j)) : [row.name, row.value].map((v, j) => /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-3 py-1.5 font-mono text-muted-foreground", children: String(v) }, j)) }, i)) })
        ] }) }) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-6 grid gap-6 md:grid-cols-3", children: categoricalCols.slice(0, 3).map((col, idx) => {
      const data = (() => {
        const counts = {};
        filteredRows.forEach((r) => {
      ...
      const activeFilterStr = [...Object.entries(filters).filter(([, v]) => Boolean(v)).map(([k2, v]) => `${k2}=${v}`), ...drillStack.map((d) => `${d.col}=${d.value}`)].join(", ");
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4", id: `secondary-chart-${idx}`, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between mb-1", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("h4", { className: "text-xs font-semibold uppercase text-foreground", children: [
              col,
              " Distribution"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[10px] text-muted-foreground mt-0.5", children: activeFilterStr ? `Filtered: ${activeFilterStr}` : "All data" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => downloadChart(`secondary-chart-${idx}`, `chart-${col}`), className: "text-xs text-muted-foreground hover:text-white", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Download, { className: "h-3 w-3" }) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 160, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data, barSize: 20, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(XAxis, { dataKey: "name", tick: {
            fill: "#94a3b8",
            fontSize: 9
          } }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: tooltipStyle, itemStyle: tooltipItemStyle }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "value", fill: COLORS[idx % COLORS.length], radius: [3, 3, 0, 0] })
        ] }) })
      ] }, col);
    }) })
  ] });
